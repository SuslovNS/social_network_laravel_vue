...
<div v-if="!isDelete">
    <div class="mb-8 pb-8 border-b border-gray-400">
        <div class="flex justify-between items-center">
            ...
            <svg v-if="this.$route.name === 'user.personal'" @click.prevent="deletePost(post)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                  stroke-width="1.5"
                  stroke="currentColor"
                  class="stroke-black-500 cursor-pointer w-6 h-6">
            ...
            <div class="flex">
                ...
                <div class="flex">
                    <svg @click.prevent="toggleLike(post)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                         stroke-width="1.5"
                         stroke="currentColor"
                         :class="['mr-2 stroke-black-500 cursor-pointer hover:fill-red-500 w-6 h-6', post.is_liked ? 'fill-red-500' : 'fill-white' ]">
                        <path stroke-linecap="round" stroke-linejoin="round"
...
<!--            <p v-if="!isShowed" @click="getComments(post)" class="text-center cursor-pointer">Show {{post.comments_count}} comments</p>-->
        ...
        <div class="mt-4">
            <div class="mb-3">
                ...
                <div>
                    <a @click.prevent="storeComment(post)" href="#" class="block p-2 w-32 text-center rounded-3xl bg-red-600 text-white
            hover:bg-white hover:border hover:border-red-600 hover:text-red-600 ml-auto">Comment</a>
                </div>
        </div>
    </div>
</div>
